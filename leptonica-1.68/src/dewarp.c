 ...
 *          built for even pages a m,or the disparity array.  The number
 ...
 */
L_DEWARP *
dewarpCreate(PIX *pixs, l_int32 pageno, l_int32 sampling, l_int32 minlines, l_int32 applyhoriz) {
	l_int32 w, h;
	L_DEWARP *dew;
...
PTAA* makeTextLineCenters(PIX* pixs, const char* morphsequence, l_int32 debugflag) {

	PROCNAME("makeTextLineCenters");

	PTAA* ptaa1;
	PTAA* ptaa2;
	PIX* pixt1,*pixt2;
	/* Make initial estimate of centers of textlines */
	ptaa1 = pixGetTextlineCenters(pixs,morphsequence, DEBUG_TEXTLINE_CENTERS);
	if (debugflag) {
...
l_int32 dewarpBuildModel(L_DEWARP *dew, l_int32 debugflag) {
	...
	PTA *pta, *ptad;
	PTAA *ptaa2, *ptaa3, *ptaa4, *ptaa5, *ptaa6, *ptaa7;
	FPIX *fpix1, *fpix2, *fpix3;

	PROCNAME("dewarpBuildModel");
	const char* const morph1 = "c15.1 + o15.1 + c100.1";
	...
	ptaa2 = makeTextLineCenters(pixs,morph1,debugflag);
	nlines = ptaaGetCount(ptaa2);
	if (nlines < dew->minlines) {
		ptaaDestroy(&ptaa2);
		ptaa2 = makeTextLineCenters(pixs,morph2,debugflag);
		if (nlines < dew->minlines) {
			ptaaDestroy(&ptaa2);
	...
	for (i = 0; i < nlines; i++) {
		...
		if (c2 <= 0) /* flat point at bottom; max value of y in curve */
			...
			/* flat point at top; min value of y in curve */
 ...
 */
PTAA *
pixGetTextlineCenters(PIX *pixs, const char* const morphsequence, l_int32 debugflag) {
	...
	//pixt1 = pixMorphSequence(pixs, "c15.1 + o15.1 + c50.1", 0);
	/* Filter to extract a vertical whitespace mask */
	pixt1 = pixMorphSequence(pixs, morphsequence, 0);
	...
	pixa2 = pixaSelectBySize(pixa1, 100, 4, L_SELECT_IF_BOTH, L_SELECT_IF_GT, 0);
 ...
 */
PTA *
pixGetMeanVerticals(PIX *pixs, l_int32 x, l_int32 y) {
 ...
 */
PTAA *
ptaaRemoveShortLines(PIX *pixs, PTAA *ptaas, l_float32 fract, l_int32 debugflag) {
 ...
 */
FPIX *
fpixBuildHorizontalDisparity(FPIX *fpixv, l_float32 factor, l_int32 *pextraw) {
	...
	for (i = 0; i < h; i++) {
		...
		for (j = 0; j < fw; j++) {
			if (j < maxloc) /* this may not work for even pages */
				...
				/* keep it at the max value the rest of the way across */
				line[j] = maxval;
		}
	}
 ...
 */
FPIX *
fpixSampledDisparity(FPIX *fpixs, l_int32 sampling) {
...
l_int32 dewarpApplyDisparity(L_DEWARP *dew, PIX *pixs, l_int32 debugflag) {
	...
	if (dew->applyhoriz) {
		if ((pixd = pixApplyHorizontalDisparity(pixv, dew->fullhdispar, dew->extraw)) == NULL)
 ...
 */
PIX *
pixApplyVerticalDisparity(PIX *pixs, FPIX *fpix) {
 ...
 */
PIX *
pixApplyHorizontalDisparity(PIX *pixs, FPIX *fpix, l_int32 extraw) {
