...
typedef bool (*PROGRESS_FUNC)(void* progress_this, int progress, int left, int right, int top, int bottom );
...
class ETEXT_DESC {             // output header
 public:
  ...
  ETEXT_DESC() : count(0), progress(0), more_to_come(0), ocr_alive(0),
                   err_code(0), cancel(NULL),progress_callback(NULL), cancel_this(NULL),progress_this(NULL) {
    end_time.tv_sec = 0;
    end_time.tv_usec = 0;
  }
