...
int runTextDetect(std::string inputImage){
    // Parse command line arguments.
    float confThreshold = 0.5;
    float nmsThreshold = 0.4;
    String modelDecoder ="/Users/renardw/dev/textfairy/frozen_east_text_detection.pb";
    //parser.get<String>("model");
    String modelRecognition = "";
    //parser.get<String>("ocr");
    ...
    VideoCapture cap;
    bool openSuccess = cap.open(inputImage);
    ...
    //1280×720
    int inpWidth =1280;
    float scaleX = (float)inpWidth / (float)frame.cols;
    int inpHeight = frame.rows * scaleX;
    inpHeight =  (32-inpHeight % 32) + inpHeight;
    ...
    std::string::size_type const p(inputImage.find_last_of('.'));
    std::string file_without_extension = inputImage.substr(0, p);
    bool saveSuccess = imwrite(file_without_extension+"_opencv.png", frame);
    CV_Assert(saveSuccess);
