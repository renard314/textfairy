...
Pix* createEdgeMask(Pix* pixs, bool debug) {
    PROCNAME("createEdgeMask");
	...
	L_INFO("mean = %f, median = %f, std dev = %f, thresh = %i\n", procName, mean, median, variance, thresh);
...
Pix* binarizeTiled(Pix* pixGrey, Pix* pixhm) {
    PROCNAME("binarize");
    bool debug = false;
	Pix* pixEdgeMask;
	...
	pixEdgeMask = createEdgeMask(pixGrey, debug);
	...
	Pix* pixb = binarizeTiled(pixGrey, tileSize);
