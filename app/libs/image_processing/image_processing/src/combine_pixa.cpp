...
void translateBoxa(Pixa* pixa, l_int32 dx, l_int32 dy, l_int32 left, l_int32 top) {
    printf("translateBoxa dx=%i dy=%i, left=%i, top=%i", dx, dy, left, top);
    bool moveDown = dy > 0;
    bool moveRight = dx > 0;
    ...
    if (moveDown || moveRight) {
        ...
        for (int j = 0; j < count; j++) {
            ...
            if (moveRight && x > left) {
                printf("moving pix %i to %i pixel to right", j, dx);
                x += dx;
            }
            if (moveDown && y > top) {
                printf("moving pix %i to %i pixel down", j, dy);
                y += dy;
            }
