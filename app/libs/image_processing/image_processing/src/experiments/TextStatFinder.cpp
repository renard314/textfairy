...
Pix* TextStatFinder::pixAnnotate(Pix* pixb, const char* textstr) {
	L_BMF *bmf;
	bmf = bmfCreate("/Users/renard/devel/leptonica/prog/fonts", 4);
...
Numa* TextStatFinder::numaDifferentiate(Numa* na) {
	Numa* result = numaCreate(0);
	l_int32 pc = numaGetCount(na);
	l_float32 prev;
	if(pc>1){
...
typedef l_float32 (*FLOAT_FUNC)(l_float32 val);

void numaApply(Numa* na, FLOAT_FUNC func){
    int n = numaGetCount(na);
    l_float32 val;
    for(int i = 0; i < n; i++){
        numaGetFValue(na, i, &val);
        numaSetValue(na, i, func(val));
    }
}
l_float32 truncate(l_float32 val){
    return L_MAX(val, 0);
}
...
bool TextStatFinder::calculateTextProbability(Pix* pix, l_int32 i, l_int32 j, l_float32* textPropability, l_float32* textSize, l_float32* lineSpacing, bool debug) {
	...
	numaPixelSum = pixSumPixelsByRow(pix, NULL);
    //numaApply(numaPixelSum, truncate);
	numaClosedPixelSum = numaDifferentiate(numaPixelSum);
    
	extrema = numaFindExtrema(numaClosedPixelSum, pixGetWidth(pix)*85, NULL);
    
    //remove all negative extrema
    //get average distance between each extrema (bottom of text line to text line bottom)
    
	extremaCount = numaGetCount(extrema);
    
    RunningStats stats;
    l_int32 lastExtrema = 0;
    for(int i = 0; i < extremaCount; i++ ) {
        l_int32 yVal, xVal;
        numaGetIValue(extrema, i, &xVal);
        numaGetIValue(numaClosedPixelSum, xVal, &yVal);
        if(yVal>0){
            if(lastExtrema>0) {
                stats.Push(xVal-lastExtrema);
            }
            lastExtrema=xVal;
        }
    }
    //return stats.Mean();
    printf("average distance from bottom to bottom = %.2f", stats.Mean());
	if (extremaCount > 1) {
...
//		}
		...
		if(debug){
			...
			numaPlot(numaClosedPixelSum, extrema,NULL, GPLOT_PNG);
...
void plotNumaAndPoint(Numa* numa, l_int32 pointIndex){
	...
	std::ostringstream name;
	std::ostringstream rootName;
	gplot = gplotCreate("numaPLotPoint", GPLOT_PNG, name.str().c_str(), "x", "y");
	std::ostringstream title;
