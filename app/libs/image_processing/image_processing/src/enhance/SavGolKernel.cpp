...
SavGolKernel::SavGolKernel(Point const& size, Point const& origin, int const hor_degree, int const vert_degree)
:	m_horDegree(hor_degree),
m_vertDegree(vert_degree),
m_width(size.x),
m_height(size.y),
m_numTerms(calcNumTerms(hor_degree, vert_degree)),
m_numDataPoints(size.x * size.y)
{
    if (hor_degree < 0) {
...
void
SavGolKernel::recalcForOrigin(Point const& origin)
{
