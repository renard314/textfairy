...
int determineThresholdForTile(Pix* pixt, bool debug) {
	...
	l_float32 iMulty;
	for (sum = 0.0, moment = 0.0, var = 0.0, countPixels = 0, i = start; i < end; i++) {
	...
	if (debug == true) {
		...
		gplotAddPlot(gplot, NULL, norm, GPLOT_LINES, "norm histogram");
	...
	if (variance<8){
		...
		return mean - variance*0.66; //0.2 when median is activated, 0.66 otherwise
	}
}
