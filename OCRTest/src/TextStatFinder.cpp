...
void TextStatFinder::numaPlot(Numa* numa, Numa* numaExtrema, l_int32 outformat) {
...
Pix* TextStatFinder::numaPlotToPix(Numa* numa, Numa* numaExtrema) {
	numaPlot(numa, numaExtrema, GPLOT_PNG);
	usleep(500000);
...
bool TextStatFinder::checkTextConditions(Numa* extrema, Numa* numaPixelSum, l_float32* textPropability, l_float32* textSize, bool debug) {
	...
	Numa *px=NULL, *py=NULL, *vx=NULL, *vy=NULL, *pa=NULL, *errors=NULL, *paw=NULL, *peakDelta=NULL, *crossings1=NULL, *crossings2=NULL;
	...
	//get the crossings
	crossings1 = numaCrossingsByThreshold(NULL, numaPixelSum, firstCrossingY);
	crossings2 = numaCrossingsByThreshold(NULL, numaPixelSum, secondCrossingY);
...
bool TextStatFinder::calculateTextProbability(Pix* pix, l_int32 i, l_int32 j, l_float32* textPropability, l_float32* textSize, l_float32* lineSpacing, bool debug) {
	...
	*lineSpacing = meanSpacing;
	...
	extrema = numaFindExtrema(numaClosedPixelSum, tileWidth / 2);
...
void TextStatFinder::numaGroup(Numa* naTextSize, Numa** groupedMean, Numa** groupedStdDev){
...
void  TextStatFinder::getTextStatsTiled(Pix* pix, Numa** textSizes){
	...
	l_float32 tileHeight = pixGetWidth(pix) / 6;
	l_float32 textPropability,textSize, lineSpacing;

	if (tileWidth < 24) {
		tileWidth = 24;
	}
	pt = pixTilingCreate(pix, 0, 0, tileWidth, tileHeight, tileWidth / 2, tileHeight / 2);
	...
	if(textSizes!=NULL){
		...
		numaGroup(naTextSize,textSizes,&textSizeStd);
