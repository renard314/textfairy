...
public class CurlView extends GLSurfaceView implements /*View.OnTouchListener,*/
	...
	private long mAnimationDurationTime = 100;
	...
	private IndexChangedObserver mIndexChangedObserver;
	public View.OnTouchListener mGestureDetecture;
		...
		if (currentTime >= mAnimationStartTime + mAnimationDurationTime) {
			if (mAnimationTargetEvent == SET_CURL_TO_RIGHT) {
				...
				if (mCurlState == CURL_LEFT) {
					setCurrentIndexInternal(mCurrentIndex-1);
				}
			...
			updatePointerPos(currentTime);
			updateCurlPos(mPointerPos);
		}
	}
	
	private void updatePointerPos(long currentTime){
		mPointerPos.mPos.set(mAnimationSource);
		float t = FloatMath.sqrt((float) (currentTime - mAnimationStartTime)/ mAnimationDurationTime);			
	...
	public void onPageSizeChanged(int width, int height) {
		...
		updateBitmaps();
		if (mIndexChangedObserver!=null){
			mIndexChangedObserver.onIndexChanged(getCurrentIndex());
		}
		requestRender();
	}
	...
	public boolean onTouchEvent(MotionEvent me) {
		int myAction = me.getAction();
		
		if (mGestureDetecture!=null && mGestureDetecture.onTouch(null,me)){
			//if a gesture is detected then the current curling should stop
			myAction = MotionEvent.ACTION_UP;

		} 
		if (mAnimate==true || mBitmapProvider == null) {
			return false;
		}
		...
		case MotionEvent.ACTION_DOWN: {
			...
			if (mViewMode == SHOW_TWO_PAGES) {
				...
				else if (mDragStartPos.x >= rightRect.left && mCurrentIndex < mBitmapProvider.getBitmapCount()) {
					mDragStartPos.x = rightRect.right;
					if (!mAllowLastPageCurl && mCurrentIndex >= mBitmapProvider.getBitmapCount() - 1) {
						return false;
					}
				...
				if (mDragStartPos.x < halfX && mCurrentIndex > 0) {
					...
					startCurl(CURL_LEFT);
				} else if (mDragStartPos.x >= halfX && mCurrentIndex < mBitmapProvider.getBitmapCount()) {
					mDragStartPos.x = rightRect.right;
					if (!mAllowLastPageCurl && mCurrentIndex >= mBitmapProvider.getBitmapCount() - 1) {
						return false;
					}
	...
	public void setCurrentIndex(int index) {
		if (mBitmapProvider == null || index <= 0) {
			...
			mCurrentIndex = Math.min(index, mBitmapProvider.getBitmapCount() - 1);
		}
		if (mIndexChangedObserver!=null){
			mIndexChangedObserver.onIndexChanged(mCurrentIndex);
		}
		updateBitmaps();
		requestRender();
	}
	...
	public void setIndexChangedObserver(IndexChangedObserver observer) {
		mIndexChangedObserver = observer;
	}
	...
	private void init(Context ctx) {
		...
		mPageLeft = new CurlMesh(20);
		mPageRight = new CurlMesh(20);
		mPageCurl = new CurlMesh(20);
	...
	private void startCurl(int page) {
		...
		case CURL_RIGHT: {
			...
			if (mCurrentIndex < mBitmapProvider.getBitmapCount() - 1) {
				Bitmap bitmap = mBitmapProvider.getBitmap(mPageBitmapWidth, mPageBitmapHeight, mCurrentIndex + 1);
	...
	private void updateBitmaps() {
		if (mBitmapProvider == null || mPageBitmapWidth <= 0 || mPageBitmapHeight <= 0) {
			return;
		}
		...
		if (rightIdx >= 0 && rightIdx < mBitmapProvider.getBitmapCount()) {
			Bitmap bitmap = mBitmapProvider.getBitmap(mPageBitmapWidth, mPageBitmapHeight, rightIdx);
			mPageRight.setBitmap(bitmap);
			mPageRight.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_RIGHT));
			mPageRight.reset();
			mRenderer.addCurlMesh(mPageRight);
		...
		if (leftIdx >= 0 && leftIdx < mBitmapProvider.getBitmapCount()) {
			Bitmap bitmap = mBitmapProvider.getBitmap(mPageBitmapWidth,	mPageBitmapHeight, leftIdx);
			mPageLeft.setBitmap(bitmap);
			mPageLeft.setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT));
			mPageLeft.reset();
			if (mRenderLeftPage) {
		...
		if (curlIdx >= 0 && curlIdx < mBitmapProvider.getBitmapCount()) {
			Bitmap bitmap = mBitmapProvider.getBitmap(mPageBitmapWidth,mPageBitmapHeight, curlIdx);
	...
	private void setCurrentIndexInternal(int index) {
		if (mIndexChangedObserver!=null) {
			mIndexChangedObserver.onIndexChanged(index);
		}
		mCurrentIndex =  index;
	}
	...
	private void updateCurlPos(PointerPosition pointerPos) {
		...
		if (mCurlState == CURL_RIGHT
			...
			float dist = FloatMath.sqrt(mCurlDir.x * mCurlDir.x + mCurlDir.y * mCurlDir.y);
	...
	public interface IndexChangedObserver {
		void onIndexChanged(int index);
		//void onBitmapSize(int w, int h, int index);
	}
