...
public class Rotate {
    ...
    public static Pix dewarp(Pix pixb, Pix pixs) {
        if (pixs == null)
            throw new IllegalArgumentException("Source pix must be non-null");

        int nativePix = nativeDewarp(pixb.mNativePix,pixs.mNativePix);
    ...
    public static Pix cleanBackground(Pix pixs) {
        if (pixs == null)
            throw new IllegalArgumentException("Source pix must be non-null");

        int nativePix = nativeCleanbackground(pixs.mNativePix);
    ...
    private static native int nativeRotateOrth(int nativePix, int quads);
    private static native int nativeDewarp(int nativePixb, int nativePix);
    private static native int nativeCleanbackground(int nativePixb);
}
